...
const handlePredict=async(req,res)=>{
    const {year,kilometers,mileage,engine,power,seats,manufacturer,fuel,transmission,owner}=req.body;
    ...
    'Volvo']
    for(let i=0; i<Manu_str.length;i++){
    ...
    console.log("trans: ",transmission," ",trans)
    let Owner=[0,0,0]
    let Owner_str=['Fourth & Above', 'Second', 'Third']
    for(let i=0; i<Owner_str.length;i++){
        if(owner===Owner_str[i]){
            Owner[i]=1;
            break;
        }
    }
    const arr=[year].concat([kilometers,mileage,engine,power,seats],Manu,Fuel,[trans],Owner)
    console.log("arr: ",arr)
    let arr1=[]
